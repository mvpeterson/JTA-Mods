...
ActionScenarioAnnotator::ActionScenarioAnnotator(std::string _output_path, const char* _file_scenario, int _max_samples, int _is_night)
{
	...
	std::string sweather;
	std::string file_scenario = "myscenario_01.txt";
	std::ifstream fin(file_scenario);
	fin >> cam_coords.x >> cam_coords.y >> cam_coords.z;
	fin >> cam_rot.x >> cam_rot.y >> cam_rot.z;
	fin >> time_h >> time_m >> time_s;
	fin >> date_m;
	fin >> sweather;
	fin.close();
	...
	GAMEPLAY::SET_OVERRIDE_WEATHER((char*)(void*)sweather.c_str());
	GAMEPLAY::SET_WEATHER_TYPE_NOW((char*)(void*)sweather.c_str());

	Entity e = PLAYER::PLAYER_PED_ID();
	ENTITY::SET_ENTITY_COORDS_NO_OFFSET(e, cam_coords.x, cam_coords.y, cam_coords.z, 0, 0, 1);
	WAIT(5000);
	ActionScenarioAnnotator::setCameraFixed(cam_coords, cam_rot, 0, fov);
//	loadScenario(file_scenario);
	...
	if (DEMO) 
		this->secondsBeforeSaveImages = 5;
	else
	...
	this->mother.setModelHash(0xECD04FE9); // cs_debra
	this->child.setModelHash(0x909D9E7F); //ig_bankman

	Vector3 spawn_point = Vector3(-285, -1898, 29.5);
	mother.spawn(Vector3(-288, -1898.0, 29.5));
	child.spawn(Vector3(-288, -1898.5, 29.5));
}

ActionScenarioAnnotator::~ActionScenarioAnnotator()
{
...
void ActionScenarioAnnotator::checkStatesAndAssignTasks()
{
	float dist;
	Vector3 aim;
	Vector3 pos;
	switch (this->mother.state_current)
	{
	case SPAWNED:
		//Vector3 target_point = Vector3(-273, -1901, 29.8);
		mother.taskWalkToTarget(Vector3(-273, -1901, 29.8));
		WAIT(1000);
		break;

	case TASK_WALK_TO_TARGET:
		if (PED::IS_PED_STOPPED(mother.ped_id))
		{
			mother.state_current = STOPPED;
		}
		break;

	case STOPPED:
		mother.taskUseMobilPhoneTimed(30000);
		break;

	case TASK_PHONE:
		current_time = std::clock() / CLOCKS_PER_SEC;
		if ((current_time - child.task_start_time) > 30* TIME_FACTOR)
		{
			pos = mother.getCurrentCoord();
			aim = child.getCurrentCoord();
			mother.taskGoToCoordWhileAimingAtCoord(pos.x, pos.y, pos.z, aim.x, aim.y, aim.z, 1.0);
		}
		break;

	case TASK_GO_TO_COORD_WHILE_AIMING_AT_COORD:
		pos = mother.getCurrentCoord();
		aim = child.getCurrentCoord();
		mother.taskGoToCoordWhileAimingAtCoord(pos.x, pos.y, pos.z, aim.x, aim.y, aim.z, 1.0);

		dist = sqrt((pos.x - aim.x) * (pos.x - aim.x) +
			(pos.y - aim.y) * (pos.y - aim.y));

		if (dist < 0.8)
		{
			mother.taskWalkToTarget(Vector3(-288, -1897, 29.8));
			WAIT(1000);
		}
	...
	switch (this->child.state_current)
	{
	case SPAWNED:
		//child.taskFollowToOffsetOfEntity(mother.ped_id, 0.3, 1.2);
		child.taskWalkToTarget(Vector3(-270, -1901, 29.8));
		WAIT(1000);
		break;

	case TASK_FOLLOW:
		aim = mother.getCurrentCoord();
		pos = child.getCurrentCoord();
	...
	case TASK_WALK_TO_TARGET:
		if (PED::IS_PED_STOPPED(child.ped_id))
		{
			child.taskChatToPed(mother.ped_id);
		}
		break;

	case TASK_CHAT:
		current_time = std::clock() / CLOCKS_PER_SEC;
		if ((current_time - child.task_start_time) > 6* TIME_FACTOR)
		{
			child.taskWanderInArea(-270, -1901, 29.8, 10.0f, 1.0f, 1.0f);
		}
		break;
	case TASK_WANDER_IN_AREA:
		current_time = std::clock() / CLOCKS_PER_SEC;
		if ((current_time - child.task_start_time) > 40* TIME_FACTOR)
		{
			child.taskFollowToOffsetOfEntity(mother.ped_id, 0.3, 1.5);
			//child.taskWanderInArea(-270, -1901, 29.8, 10.0f, 1.0f, 1.0f);
		}
		break;
...
int ActionScenarioAnnotator::update()
{
...
void ActionScenarioAnnotator::addwPed(Ped p, Vector3 from, Vector3 to, int stop, float spd)
{
	if (nwPeds > 299)
		return;
...
void ActionScenarioAnnotator::addwPed_scenario(Ped p)
{
...
void ActionScenarioAnnotator::get_2D_from_3D(Vector3 v, float *x2d, float *y2d) {
...
void ActionScenarioAnnotator::save_frame() {
...
void ActionScenarioAnnotator::setCameraMoving(Vector3 A, Vector3 B, Vector3 C, int fov) {
...
void ActionScenarioAnnotator::setCameraFixed(Vector3 coords, Vector3 rot, float cam_z, int fov) {
...
void ActionScenarioAnnotator::spawnPed(Vector3 pos, int numPed) {
...
Vector3 ActionScenarioAnnotator::teleportPlayer(Vector3 pos){
...
void ActionScenarioAnnotator::draw_text(char *text, float x, float y, float scale) {
	UI::SET_TEXT_FONT(0);
...
int ActionScenarioAnnotator::myreadLine(FILE *f, Vector3 *pos, int *nPeds, int *ngroup, int *currentBehaviour, float *speed, Vector3 *goFrom, Vector3 *goTo, int *task_time, int *type, 
...
Cam ActionScenarioAnnotator::lockCam(Vector3 pos, Vector3 rot) {
	CAM::DESTROY_ALL_CAMS(true);
...
void ActionScenarioAnnotator::loadScenario(const char* fname)
{
	...
	if (moving == 0)
		ActionScenarioAnnotator::setCameraFixed(cCoords, cRot, 0, fov);
	else
		ActionScenarioAnnotator::setCameraMoving(A, B, C, fov);
}

void ActionScenarioAnnotator::spawn_peds_flow(Vector3 pos, Vector3 goFrom, Vector3 goTo, int npeds, int ngroup, int currentBehaviour, 
...
void ActionScenarioAnnotator::spawn_peds(Vector3 pos, Vector3 goFrom, Vector3 goTo, int npeds, int ngroup, int currentBehaviour,
	int task_time, int type, int radius, int min_lenght, int time_between_walks, int spawning_radius, float speed) {
	...
	for (int i = 0; i < npeds; i++) 
	{
		...
		/*Hash modelHash = 0x909D9E7F;
		STREAMING::REQUEST_MODEL(modelHash);
		while (!STREAMING::HAS_MODEL_LOADED(modelHash))
			WAIT(0);

		ped[i] = PED::CREATE_PED(NULL, modelHash, pos.x, pos.y, pos.z, 0.0, false, false);
...
void ActionScenarioAnnotator::walking_peds()
{
	for (int i = 0; i < nwPeds; i++)
	{
